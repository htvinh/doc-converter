...
| `config.py` | Logging, `TEMP_DIR = Path("temp_files")` |
| `converters/pandoc_converter.py` | `.md`/`.html`/`.htm`/`.txt` → `.docx` via pypandoc + python-docx |
| `converters/docling_converter.py` | `.docx`/`.doc`/`.xls`/`.xlsx`/`.csv`/`.pdf` → `.md` via MarkItDown |
...
- Input ends in `.docx`/`.doc`/`.xls`/`.xlsx`/`.csv`/`.pdf` → **MarkItDown** path → output `.md`
...
- **MarkItDown** is lightweight (no singleton needed) — replaces the old Docling path. Requires `pip install markitdown[all]` for full format support.
...
```
streamlit, markitdown[all], pypandoc, pathlib, python-docx, requests   (requirements.txt)
